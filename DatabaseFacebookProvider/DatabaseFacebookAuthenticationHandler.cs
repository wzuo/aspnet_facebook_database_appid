...
{
    public class DatabaseFacebookAuthenticationHandler : AuthenticationHandler<DatabaseFacebookAuthenticationOptions>
    {
        ...
        public DatabaseFacebookAuthenticationHandler(HttpClient httpClient, ILogger logger)
        {
        ...
        protected override async Task<AuthenticationTicket> AuthenticateCoreAsync()
        {
            ...
            try
            {
                ...
                var tokenRequest = "grant_type=authorization_code" +
                    ...
                    "&client_id=" + Uri.EscapeDataString(DatabaseFacebookAuthenticationOptions.AppId) +
                    "&client_secret=" + Uri.EscapeDataString(DatabaseFacebookAuthenticationOptions.AppSecret);

                var tokenResponse = await _httpClient.GetAsync(TokenEndpoint + "?" + tokenRequest, Request.CallCancelled);
        ...
        {
            using (var algorithm = new HMACSHA256(Encoding.ASCII.GetBytes(DatabaseFacebookAuthenticationOptions.AppSecret)))
            {
                var hash = algorithm.ComputeHash(Encoding.ASCII.GetBytes(accessToken));
                var builder = new StringBuilder();
                for (var i = 0; i < hash.Length; i++)
                {
                    builder.Append(i.ToString("x2", CultureInfo.InvariantCulture));
                }
